...
function shinsegaeLabelValues(item: Label | QueueItem) {
  const values = templateValues(item).map(value => value.trim()).filter(Boolean);
  // Some UniLabel files store the code first and others store the title first.
  const code = values.find(value => !/[가-힣]/.test(value) && /^[a-z0-9-]+$/i.test(value)) || '';
  const name = values.find(value => /[가-힣]/.test(value)) || values.find(value => value !== code) || item.product_name;
  return {code, name};
}
...
function displayProductName(item: Label) {
  ...
  if (item.vendor === SHINSEGAE_VENDOR) return shinsegaeLabelValues(item).name;
...
function labelMarkup(item: QueueItem, copy: number) {
  ...
  if (item.vendor === SHINSEGAE_VENDOR) {
    const shinsegae = shinsegaeLabelValues(item);
    return `<article class="label shinsegae-myeongdong" data-copy="${copy}"><div class="shinsegae-code" style="font-size:${fitFont(shinsegae.code,10,6.2,18)}pt">${escapeHtml(shinsegae.code)}</div><div class="shinsegae-title" style="font-size:${fitFont(shinsegae.name,10,5.8,31)}pt">${escapeHtml(shinsegae.name)}</div></article>`;
  }
