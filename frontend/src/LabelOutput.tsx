...
export default function LabelOutput() {
  ...
  const syncRetailBarcodes=async(selectedVendor=vendor)=>{const response=await fetch('/api/labels/sync-product-barcodes',{method:'POST'});const data=await response.json();if(!response.ok){setMessage(data.message||'셀메이트 바코드 동기화에 실패했습니다.');return}setMessage(`셀메이트 기준으로 기존 ${data.updated}개를 교체하고 새 라벨 ${data.created}개를 추가했습니다. 이미 일치 ${data.alreadyCurrent}개, 코드 확인 필요 ${data.missing}개입니다.`);await loadVendors();const refreshed=await fetch(`/api/labels?vendor=${encodeURIComponent(selectedVendor)}&search=${encodeURIComponent(search)}`);setLabels(await refreshed.json())};
