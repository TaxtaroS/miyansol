import {matchProduct,readOrderFile,rowsFromText,type ParsedOrderRow} from './order-reader.js';
import {readOrderWithGemini} from './gemini-order-reader.js';
...
export async function analyzeOrderFile(file:{buffer:Buffer;mimetype:string;originalname:string},products:AnalysisProduct[],browserOcrText?:string){
  ...
  const items:AnalyzedOrderItem[]=extracted.rows.map(row=>{
    ...
    return {sourceName:row.note?`${row.name} [${row.note}]`:row.name,quantity:row.quantity,productId:row.needsReview?null:matched?.id??null,confidence:row.needsReview?0:matched?.score??0};
  });
  return {
