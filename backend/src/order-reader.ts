...
async function readPdfWithPaperMate(buffer:Buffer,filename:string){
  const configuredHost=process.env.DOCUMENT_WORKER_HOST?.trim();
  const host = configuredHost || process.env.DOCUMENT_WORKER_URL || process.env.VERCEL_PROJECT_PRODUCTION_URL || process.env.VERCEL_URL || 'http://localhost:8000';
  if (!host) return null;
  ...
  try {
    const response = await fetch(`${baseUrl.replace(/\/$/, '')}/document-api/extract-pdf`, { method: 'POST', body, signal: AbortSignal.timeout(30000) });
