...
(async()=>{
  ...
  try{
    ...
    await page.getByRole('button',{name:'패킹 출고',exact:true}).click();
    ...
    await page.waitForFunction(()=>document.querySelector('.order-upload-panel [role="status"]')?.textContent?.includes('2개 주문서 저장'));
